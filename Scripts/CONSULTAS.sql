...
INSERT INTO lista (nome,data_hora_crt,data_hora_mod,usuario_id_mod,usuario_id) VALUES ("", NOW(), NOW(), 0, 0);
-- Cria uma nova lista
UPDATE lista SET nome = "", data_hora_mod = NOW(), usuario_id_mod = 0 WHERE lista_id= 0;
-- Edita uma lista, atuzalizando a data e o usuário que modificou
SELECT u.usuario_id,l.lista_id,l.nome,l.data_hora_crt,l.data_hora_mod FROM usuario AS u JOIN lista AS l on l.usuario_id = u.usuario_id WHERE u.usuario_id = 0;
-- Seleciona listas criadas por um usuário
SELECT l.lista_id,l.nome,l.data_hora_crt,l.data_hora_mod,uc.nome AS criador,umod.nome AS ultima_mod FROM Lista AS l JOIN Convidado AS c ON l.lista_id = c.lista_id JOIN Usuario AS uc ON uc.usuario_id = l.usuario_id JOIN Usuario AS umod ON umod.usuario_id = l.usuario_id_mod WHERE c.usuario_id = 0 AND c.estado_convite = 1;
-- Seleciona listas compartilhadas com um usuário